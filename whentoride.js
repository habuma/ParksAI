...
const getCurrentWaitTime = async(attractionName) => {
    ...
    const forecastedWaitTimes = data.liveData[0].forecast.filter(forecastItem => {
        return new Date(forecastItem.time) > Date.now();
    });
    return JSON.stringify({
        attractionName: attractionName,
        waitTime: waitTime,
        status: status,
        forecastedWaitTimes: forecastedWaitTimes
    });
}

const askForWaitTime = async (attractionName) => {
    const messages = [
        { role: 'user', content: `Based on the forecasted wait times, when is the best time to ride ${attractionName}? If the status is not OPERATING then say that the attraction is closed. If you do not know say "I do not know".`}
    ];
